    ...
    public class COMPortsData
    {
        ...
        public static List<DeviceInfo> GetAllCOMPorts()
        {
            ...
            for (int index = 0; index < guids.Length; index++)
            {
                ...
                {
                    throw new Exception("Невозможно получить информацию о COM-портах");
                }
                ...
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        var (vid, pid) = GetVidPidFromComPort(deviceInfo.Name);
                        deviceInfo.VID = vid;
                        deviceInfo.PID = pid;
        ...
        public static (string VID, string PID) GetVidPidFromComPort(string comPort)
        {
            if (string.IsNullOrWhiteSpace(comPort))
                throw new ArgumentException("COM port name cannot be empty", nameof(comPort));

            // Добавляем префикс для корректного формата (например, "COM3" -> "\\.\COM3")
            string formattedPort = comPort.StartsWith("\\") ? comPort : $"\\\\.\\{comPort}";
                ...
                "SELECT * FROM Win32_PnPEntity WHERE Caption LIKE '%(COM%'"))
            {
                foreach (ManagementObject queryObj in searcher.Get())
                {
                    ...
                    if (caption != null && caption.Contains($"({comPort})"))
                    {
                        // Ищем VID и PID в DeviceID (формат VID_XXXX или PID_XXXX)
                        ...
                        // Возвращаем "N/A", если не найдено
                        string vid = vidMatch.Success ? vidMatch.Groups[1].Value : "N/A";
                        string pid = pidMatch.Success ? pidMatch.Groups[1].Value : "N/A";
            ...
            return ("N/A", "N/A"); // Если COM-порт не найден
        }
